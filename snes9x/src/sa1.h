...
typedef struct
{
   const SOpcodes* S9xOpcodes;   /* pico port: tables are const (flash) */
   uint8_t       _Carry;
   uint8_t       _Zero;
   uint8_t       _Negative;
   uint8_t       _Overflow;
...
/* Pico port: SA-1 char-conversion scratch buffer (64 KB). Upstream reuses the
 * tail of a fixed MAX_ROM_SIZE ROM buffer (Memory.ROM[MAX_ROM_SIZE-0x10000]);
 * here Memory.ROM is the exact-size cart image in PSRAM with no slack, so a
 * dedicated PSRAM buffer is used instead. Allocated once in SA1ROMMap. */
extern uint8_t* SA1CharDMABuffer;
