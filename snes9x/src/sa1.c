...
/* Pico port: dedicated 64 KB char-conversion scratch buffer (see sa1.h).
 * Allocated in SA1ROMMap (memmap.c). Replaces Memory.ROM[MAX_ROM_SIZE-0x10000]. */
uint8_t* SA1CharDMABuffer = NULL;
...
void S9xSetSA1(uint8_t byte, uint32_t address)
{
   ...
   case 0x224f:
      ...
      if ((Memory.FillRAM [0x2230] & 0xb0) == 0xa0)
      {
         ...
         memcpy(SA1CharDMABuffer + (SA1.in_char_dma << 4), &Memory.FillRAM [0x2240], 16);
...
static void S9xSA1CharConv2(void)
{
   ...
   uint8_t* q = SA1CharDMABuffer + offset * 64;
   int l, b;
