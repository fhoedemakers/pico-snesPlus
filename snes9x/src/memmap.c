...
/**********************************************************************************************/
/* Pico port: SNES buffers are partitioned between internal SRAM (hot working
 * set) and external PSRAM (cold/scratch). See port_alloc.h. Memory.ROM is
...
bool S9xInitMemory(void)
{
    ...
    * cpuexec) live in SRAM via CMake .time_critical placement instead —
    * the speed win from running ~150 KB of branchy interpreter code from
    * SRAM dwarfs what holding Memory.RAM / Memory.Map in SRAM gave us. */
    ...
    * PSRAM cache amortizes well. Frees SRAM headroom for cpuops. */
   IPPU.ScreenColors = (uint16_t *)port_alloc_psram(256 * 9 * sizeof(uint16_t));
   if (IPPU.ScreenColors) memset(IPPU.ScreenColors, 0, 256 * 9 * sizeof(uint16_t));
   IPPU.DirectColors = IPPU.ScreenColors + 256;
   IPPU.TileCache  = (uint8_t*)port_alloc_psram(MAX_2BIT_TILES * 128);      /* ~512 KB */
   IPPU.TileCached = (uint8_t*)port_alloc_psram(MAX_2BIT_TILES);            /* 4 KB flags → PSRAM */
   if (IPPU.TileCache)  memset(IPPU.TileCache,  0, MAX_2BIT_TILES * 128);
   if (IPPU.TileCached) memset(IPPU.TileCached, 0, MAX_2BIT_TILES);

   bytes0x2000 = (uint8_t *)port_alloc_psram(0x2000);                       /* 8 KB unmapped-region placeholder → PSRAM */

   /* Memory.ROM is provided by the caller from PSRAM. */
...
void S9xDeinitMemory(void)
{
   ...
   port_alloc_free(Memory.VRAM);    Memory.VRAM = NULL;
   ...
   port_alloc_free(bytes0x2000); bytes0x2000 = NULL;

   /* Memory.ROM lifetime is owned by the caller. */
   Memory.ROM = NULL;
   Memory.ROM_Offset = 0;
