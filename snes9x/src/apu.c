...
bool S9xInitAPU()
{
    ...
    * IRAM keeps the audio CPU inner loop entirely off PSRAM. The 64 KB
    * cost is the heap's biggest single allocation but it fits easily
    * since all other emulator buffers (Memory.RAM, Memory.Map, VRAM,
    * SRAM, FillRAM, TileCache, ...) live in PSRAM. */
   IAPU.RAM = (uint8_t*) port_alloc_sram(0x10000);
