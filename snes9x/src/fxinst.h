...
/* Pico port SuperFX rework: the fetch/ROM-read below index a bank pointer with
 * a full 16-bit offset, so each bank must present a 64 KB window. Rather than
 * build CATSFC's ~6 MB mirrored pvRom (won't fit this board's 8 MB PSRAM),
 * apvRomBank[] points straight into the linear cart ROM (see FxReset) and the
 * offset is folded here: banks 0x00-0x3f are LoROM (32 KB mirrored into the
 * 64 KB window -> mask 0x7fff); banks 0x40-0x5f and RAM banks 0x70+ are 64 KB
