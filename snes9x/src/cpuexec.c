 ...
 * (the actual opcode bodies) stays in flash — moving it to SRAM costs
 * 42 KB which overflows the heap budget. */
#if defined(__has_include) && __has_include("pico.h")
#include "pico.h"
#endif
...
void __not_in_flash_func(S9xDoHBlankProcessing)()
{
   CPU.WaitCounter++;
