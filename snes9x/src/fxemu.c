...
void FxReset(FxInit_s* psFxInfo)
{
   ...
   /* Make ROM bank table. Pico port: pvRom is the linear cart ROM
    * (Memory.ROM), NOT CATSFC's ~6 MB mirrored buffer. Each apvRomBank[]
    * entry points at the start of the bank's data in linear ROM; the 32 KB
    * LoROM fold for low banks is applied by FX_BANKMASK in the ROM/PRGBANK
    * macros. nb32 = number of 32 KB banks; nb64 = number of 64 KB banks.
    * Indices wrap within the real ROM so no access runs past pvRom. */
   {
      uint32_t nb32 = GSU.nRomBanks ? GSU.nRomBanks : 1;
      uint32_t nb64 = nb32 >> 1;
      if (nb64 == 0)
         nb64 = 1;
      for (i = 0; i < 256; i++)
      {
         uint32_t b = i & 0x7f;
         if (b < 0x40) /* LoROM: 32 KB banks (offset folded to 0x7fff) */
            GSU.apvRomBank[i] = &GSU.pvRom[(b % nb32) * 0x8000];
         else          /* HiROM-linear: 64 KB banks */
            GSU.apvRomBank[i] = &GSU.pvRom[((b - 0x40) % nb64) * 0x10000];
      }
   }
...
#if SUPERFX_DEBUG
   sfx_calls++;
   /* Remember every SFR bit the CPU ever set, so we can tell "GO never set"
